...
public class Design04_EmployeeImportance {
	...
	class Solution {
		public int getImportance(List<Employee> employees, int id) {
			HashMap<Integer, Employee> mapEmployee = new HashMap<Integer, Employee>();

			for (Employee emp : employees) {
				mapEmployee.put(emp.id, emp);
			}

			return getImportance_fromMap(mapEmployee, id);
		}

		private int getImportance_fromMap(HashMap<Integer, Employee> mapEmployee, int id) {
			int result = mapEmployee.get(id).importance;
			for (Integer emp_id : mapEmployee.get(id).subordinates) {
				result += getImportance_fromMap(mapEmployee, emp_id);
			}
			return result;
		}
	}
