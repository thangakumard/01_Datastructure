...
```python
# Example: Connection pool (usually automatic)
...
class RateLimiterGateway:
    def __init__(self, zk_client):
        ...
        self.zk.watch("/rate_limiter/rules", 
                     callback=self.on_rules_changed)
    
    def load_rules(self):
        rules_json = self.zk.get("/rate_limiter/rules")[0]
        return json.loads(rules_json)
    
    def on_rules_changed(self, event):
        """Callback when rules change in Zookeeper"""
        self.rules = self.load_rules()
        logger.info(f"Rules updated: {self.rules}")
    
    def check_rate_limit(self, client_id, endpoint):
        # Use self.rules (always fresh, no network call)
        applicable_rules = self.find_rules(client_id, endpoint)
        return self.check_tokens(client_id, applicable_rules)
```
...
**Execution in Python:**
```python
script = redis_client.register_script(lua_script_content)

result = script(
    keys=['alice'],
    args=[10, 100, time.time()]
)
# Returns [1, 89, 100]  → [allowed, remaining, limit]
```
